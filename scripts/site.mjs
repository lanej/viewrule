...
const ruleExamples = {
  ...
  "DR-008": {
    ...
    interactive: "../examples/#drawers",
  },
  "DR-009": {
    good: "A failed refresh retains the last known snapshot, labels it stale, and distinguishes its as-of time from the failed attempt.",
    bad: "The failed request becomes zero shipments and appears current.",
    interactive: "../examples/behavior.html?rule=DR-009",
  },
  "DR-010": {
    good: "Opening and closing shipment detail preserves the filter, selected shipment identity, and useful focus position.",
    bad: "Returning clears the filter and silently selects whichever row now occupies the old index.",
    interactive: "../examples/behavior.html?rule=DR-010",
  },
  "DR-011": {
    good: "Publish rates for 12 selected lanes names both the action and affected scope before commitment.",
    bad: "Apply gives no indication of what will change or how much of the dataset is affected.",
    interactive: "../examples/behavior.html?rule=DR-011",
  },
  "DR-012": {
    good: "A rejected proposal keeps the entered value, explains the correction, and a reversible action offers a real Undo.",
    bad: "Failure clears the work and shows only Something went wrong.",
    interactive: "../examples/behavior.html?rule=DR-012",
  },
  "DR-013": {
    good: "The three exceptions requiring action lead the screen; the 1,200-shipment total remains available but subordinate.",
    bad: "A giant aggregate KPI dominates while the actionable exceptions are visually buried.",
    interactive: "../examples/behavior.html?rule=DR-013",
  },
  "DR-014": {
    good: "Tab reaches the disclosure, Enter opens it, and Escape closes it while keeping focus predictable.",
    bad: "Essential evidence appears only on pointer hover.",
    interactive: "../examples/behavior.html?rule=DR-014",
  },
  "DR-015": {
    good: "Long service names wrap or reflow so distinguishing suffixes remain visible with enlarged text.",
    bad: "Different services truncate to the same prefix, making them indistinguishable.",
    interactive: "../examples/behavior.html?rule=DR-015",
  },
  "DR-016": {
    good: "Unordered carriers use qualitative colors; an ordered magnitude uses a sequential scale.",
    bad: "A light-to-dark ramp is used for unordered carrier identities, implying a ranking that does not exist.",
    interactive: "../examples/behavior.html?rule=DR-016",
  },
};
...
for (const rule of policy.rules) {
  ...
  const sections = sectionsFrom(rule.body);
  ruleCards.push(
...
<article class="example good"><span class="example-label">Good</span><p>${escapeHtml(example.good)}</p></article>
<article class="example bad"><span class="example-label">Bad</span><p>${escapeHtml(example.bad)}</p></article>
</div>
${interactive}
</section>
<section>
...
await writeFile(
  ...
  `:root{color-scheme:light dark;--paper:light-dark(#fff,#17241f);--canvas:light-dark(#f7f9f7,#111b16);--ink:light-dark(#172e28,#edf5f1);--secondary:light-dark(#4d6458,#b7cabe);--line:light-dark(#d4dfd7,#42584a);--soft:light-dark(#eef4ef,#25382c);--brand:light-dark(#24583d,#aad2b8);--bad:light-dark(#8a4b20,#e7b18c);font:15px/1.55 system-ui,sans-serif;color:var(--ink);background:var(--canvas)}*{box-sizing:border-box}body{margin:0}main{max-width:900px;margin:0 auto;padding:32px 24px 64px}a{color:var(--brand);text-underline-offset:3px}.crumb{margin-bottom:36px;color:var(--secondary)}header{margin-bottom:36px}h1{font-size:clamp(30px,5vw,48px);line-height:1.05;letter-spacing:-.03em;margin:12px 0}h2{font-size:18px;margin:32px 0 8px}p{max-width:70ch}.lede{font-size:17px;color:var(--secondary)}.rule-meta{display:flex;gap:10px;align-items:center;font-size:12px;text-transform:uppercase;letter-spacing:.08em}.mode{border:1px solid var(--line);border-radius:999px;padding:3px 8px;background:var(--paper)}.example-grid{display:grid;grid-template-columns:1fr 1fr;border:1px solid var(--line);border-radius:8px;overflow:hidden;background:var(--paper)}.example{padding:22px}.example+.example{border-left:1px solid var(--line)}.example-label{font-weight:650}.good .example-label{color:var(--brand)}.bad .example-label{color:var(--bad)}.example-link{margin-top:14px}.rule-nav{display:grid;grid-template-columns:1fr auto 1fr;gap:20px;margin-top:48px;padding-top:20px;border-top:1px solid var(--line)}.rule-nav>:last-child{text-align:right}.rule-list{display:grid;gap:10px}.rule-card{display:grid;grid-template-columns:72px 1fr auto;gap:14px;align-items:center;padding:16px 18px;border:1px solid var(--line);border-radius:7px;background:var(--paper);text-decoration:none}.rule-card:hover{border-color:var(--brand)}.rule-card span,.rule-card small{color:var(--secondary)}code{background:var(--soft);padding:1px 4px;border-radius:3px}@media(max-width:650px){main{padding:24px 18px 48px}.example-grid{grid-template-columns:1fr}.example+.example{border-left:0;border-top:1px solid var(--line)}.rule-card{grid-template-columns:64px 1fr}.rule-card small{grid-column:2}.rule-nav{font-size:13px}}`,
);
