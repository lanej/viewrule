...
try {
  ...
  const pin = JSON.parse(await readFile(path.join(root, "plugins/claude-code/engine.json")));
  if (process.env.VIEWRULE_VERIFY_PLUGIN_PIN === "true" && pin.version === packed.version && pin.sha256 !== digest)
    throw new Error(`Plugin pin does not match packed engine: expected ${pin.sha256}, packed ${digest}. Update the pin from the tested archive.`);
