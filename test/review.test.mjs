...
test("Viewrule cites design violations, learns feedback, and accepts a repaired responsive comparison", { timeout: 60000 }, async (t) => {
  ...
  const launcher = process.env.VIEWRULE_TEST_BIN ?? path.resolve(import.meta.dirname, "../bin/viewrule.mjs");
  const cli = (args, input = "") => new Promise((resolve, reject) => {
    const child = execFile(launcher, args, {
      cwd: project,
      env: { ...process.env, VIEWRULE_CONFIG_DIR: globalDir },
      timeout: 30000,
    }, (error, stdout, stderr) => {
