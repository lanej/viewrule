...
test(
  ...
  async (t) => {
    ...
    const latestAfterGood = JSON.parse(
    ...
    const installedPackageText = await readFile(installedPackagePath, "utf8");
    ...
    assert.ok(
      mismatchedHook.reason.includes(
        `generated by viewrule engine v${pin.version}`,
      ),
    );
    assert.ok(
      mismatchedHook.reason.includes("Stop hook is running v9.9.9-test"),
    );
    await writeFile(installedPackagePath, installedPackageText);
    assert.deepEqual(
      await hook(),
      {},
