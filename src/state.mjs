...
export async function hookDecision(payload, globalDir) {
  ...
  try {
    ...
    if (latest?.engineVersion && latest.engineVersion !== currentEngineVersion)
      return {
        decision: "block",
        reason: `UI review state was generated by viewrule engine v${latest.engineVersion}, but this Stop hook is running v${currentEngineVersion}. Use the same viewrule engine for check and hook. If this followed a plugin update, restart Claude Code, then rerun viewrule check.`,
      };
    if (
      latest?.status === "pass" &&
