...
export async function runPriorityCheckpoint(root) {
  ...
  try {
    ...
    for (const [name, viewport, enlarged] of scenarios) {
      ...
      try {
          ...
          ({ initialize, enlarged }) => {
            ...
            for (const quality of ["good", "bad"]) {
              ...
              const template = /** @type {HTMLTemplateElement} */ (
                document.querySelector("#scene-DR-013")
              );
        ...
        const observations = await page
          ...
          .evaluateAll((samples) =>
            samples.map((sample) => {
              ...
              const sections = {};
              for (const section of sample.querySelectorAll(
                "[data-priority]",
              )) {
                sections[section.getAttribute("data-priority")] = {
                  text: section.textContent.replace(/\s+/g, " ").trim(),
                  box: box(section),
                  styles: [section, ...section.querySelectorAll("*")].map(
                    (element) => {
                      const s =
              ...
              return {
                ...
                order: [...sample.querySelectorAll("[data-priority]")].map(
                  (section) => section.getAttribute("data-priority"),
                ),
                clipped: [
                  ...sample.querySelectorAll("h4, h5, p, button, strong"),
                ].filter(
                  (el) =>
        ...
        for (const key of ["exceptions", "volume"]) {
          assert.equal(
            good.sections[key].text,
            bad.sections[key].text,
            `${name}: same ${key} content`,
          );
          assert.deepEqual(
            good.sections[key].styles,
            bad.sections[key].styles,
            `${name}: same ${key} styles`,
          );
          for (const dimension of ["width", "height"])
            assert.ok(
              Math.abs(
                good.sections[key].box[dimension] -
                  bad.sections[key].box[dimension],
              ) <= 1,
              `${name}: same ${key} ${dimension}`,
            );
        }
        assert.deepEqual(good.order, ["exceptions", "volume"]);
        assert.deepEqual(bad.order, ["volume", "exceptions"]);
        assert.ok(
          good.sections.exceptions.box.bottom <= good.sections.volume.box.top,
        );
        assert.ok(
          bad.sections.volume.box.bottom <= bad.sections.exceptions.box.top,
        );
        assert.equal(
          good.clipped + bad.clipped,
          0,
          `${name}: no clipped critical content`,
        );
        assert.equal(
          await page
            .locator("html")
    ...
    for (const [id, assessment] of Object.entries(entry.rules)) {
      if (["automated", "behavioral"].includes(assessment.evidenceType)) {
        assert.ok(
          ...
          `${id} must name executed checkpoint evidence`,
        );
        for (const name of assessment.checks)
          assert.ok(
            checks.every((scenario) => scenario.passed.includes(name)),
            `${id}/${name} must execute successfully in both layouts`,
          );
      }
    }
    const report = {
      ...
      rendering:
        "Actual component template, shared CSS and initializer mounted offline. Integration is covered by the existing installed-package walkthrough.",
      ...
      captures,
      limits:
        "125% component text is not browser zoom or a complete accessibility audit. Authored order is mechanically checked; human review must judge priority and all other declared subjective requirements.",
    };
    await writeFile(
    ...
    console.log(
      "DR-013 checkpoint passed: equal content/style/geometry, order-only mutation, keyboard disclosure, and four captures.",
    );
  } finally {
    await browser.close();
  }
