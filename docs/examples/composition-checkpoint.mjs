  ...
  const capture = async (name, variant = "good", activeRules = rules) => {
    ...
    try {
      ...
      await page.evaluate((variant) => {
        if (variant === "stretched") document.body.dataset.stretched = "true";
        if (variant === "bad-peers") document.body.dataset.badPeers = "true";
        ...
        if (variant === "tiny")
          document.querySelector("#offers").setAttribute("style", "font-size: 8px");
